...
if [[ -z "${CONJUR_APPLIANCE_URL}" ]]; then
  echo "You must set the CONJUR_APPLIANCE_URL env var."
  all_set=false
fi
...
if [[ -z "${AUTHN_USERNAME}" ]]; then
  echo "You must set the AUTHN_USERNAME env var."
  all_set=false
fi
if [[ -z "${AUTHN_PASSWORD}" ]]; then
  echo "You must set the AUTHN_PASSWORD env var."
  all_set=false
fi
if ! $all_set; then
  exit -1
